...
G(f)=G(-f)=\conj{G(f)},
\]
so $G(f)$ equals its own conjugate, i.e.\ $G$ is \textbf{real}; and $G(f)=G(-f)$ says $G$ is \textbf{even}. (This is exactly why a valid SDF, the transform of the real even ACVS, is real and even.)
...
\textbf{(b)} Show that $\abs{D_n(f)}$ vanishes exactly at the nonzero \textbf{Fourier frequencies} $f_k=k/n$, $k=1,\dots,n-1$, while $D_n(0)=n$. (This is why the periodogram of a pure sinusoid at a Fourier frequency does not leak onto the other Fourier frequencies.) \hfill(1.5 pts)

\textbf{(c)} \emph{(Aliasing / Nyquist.)} Let $\{X_t\}$ be obtained by sampling a continuous-time process at interval $\Delta$, and recall the aliasing identity for the sampled spectral density,
\[
\sdf(f)=\sum_{k\in\Ints}\sdf_{\mathrm{c}}\!\left(f+\frac{k}{\Delta}\right),
\qquad \abs f\le \frac{1}{2\Delta}.
\]
Suppose the continuous SDF is band-limited triangular,
$\sdf_{\mathrm{c}}(f)=(1-\abs f)\,\ind_{\{\abs f\le 1\}}$.
For $\Delta=1$ (Nyquist frequency $\tfrac12$) compute the aliased SDF $\sdf(f)$ on $\abs f\le\tfrac12$, and state whether aliasing has occurred and why. \hfill(1.5 pts)
\statementend
...
\textbf{(c)} With $\Delta=1$ the Nyquist frequency is $\tfrac12$ and the aliasing identity reads $\sdf(f)=\sum_{k\in\Ints}\sdf_{\mathrm{c}}(f+k)$ on $\abs f\le\tfrac12$. Since $\sdf_{\mathrm{c}}$ is supported on $[-1,1]$, for $\abs f\le\tfrac12$ only $k\in\{-1,0,1\}$ can contribute:
\[
\sdf(f)=\sdf_{\mathrm{c}}(f-1)+\sdf_{\mathrm{c}}(f)+\sdf_{\mathrm{c}}(f+1).
\]
Evaluate each term on $\abs f\le\tfrac12$:
\begin{itemize}
  \item $\sdf_{\mathrm{c}}(f)=1-\abs f$ (always in support);
  \item $\sdf_{\mathrm{c}}(f+1)=1-\abs{f+1}$ is nonzero only when $\abs{f+1}\le1$, i.e.\ $f\le0$, where it equals $1-(1+f)=-f$;
  \item $\sdf_{\mathrm{c}}(f-1)=1-\abs{f-1}$ is nonzero only when $\abs{f-1}\le1$, i.e.\ $f\ge0$, where it equals $1-(1-f)=f$.
\end{itemize}
Hence
\[
\sdf(f)=
...
\end{cases}
\]
that is, $\boxed{\,\sdf(f)\equiv1\ \text{ on }\abs f\le\tfrac12.\,}$
\textbf{Aliasing has occurred.} The continuous spectrum had support out to $\abs f=1$, i.e.\ \emph{beyond} the Nyquist frequency $\tfrac12$; sampling at $\Delta=1$ folds that excess high-frequency power back into the band $[-\tfrac12,\tfrac12]$. Here the folded triangular tails fill in exactly the missing area, turning a triangular continuous SDF into a \emph{flat} (white-looking) sampled SDF --- the high-frequency content has been irretrievably mixed into the low frequencies.
